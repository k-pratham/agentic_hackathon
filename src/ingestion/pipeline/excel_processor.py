...
def parse_compliance_excel(file_path: str) -> list[dict]:
    """
    Parse a compliance excel file.
    
    Supports both standard horizontal format (rows are data points)
    and transposed format (columns are data points, rows are fields).
    
    Returns:
        List of dicts representing each compliance observation, with keys:
          - supervised_entity_name
          - sr_no
          - action_para_id
          - observation
          - actual_due_date
          ...
          - status
          - raw_data (full dict)
          - text_content (concatenated text representation for embeddings)
    """
    ...
    try:
        ...
        # Gather column A values to check if transposed
        col_a_vals = []
        ...
        # Indicators of transposed format
        transposed_indicators = [
            "supervised entity name",
            "action para id",
            "action para description",
            "se status",
            "sr.no."
        ]
        ...
        if is_transposed:
            ...
            # Build keys from Column A
            keys = []
            for r in range(1, max_r + 1):
                val = ws.cell(row=r, column=1).value
                keys.append(str(val).strip() if val is not None else f"Field_{r}")

            # Each column from 2 (B) onwards is a record
            for c in range(2, max_c + 1):
                ...
                for r, key in enumerate(keys):
                    val = col_vals[r]
                    record_data[key] = str(val).strip() if val is not None else ""
            ...
            for r in range(2, max_r + 1):
                ...
                for c, header in enumerate(headers):
                    val = row_vals[c]
                    record_data[header] = str(val).strip() if val is not None else ""

                records.append(record_data)

        # Standardize and extract structured text context
        parsed_records: list[dict] = []
        for rec in records:
            try:
                # Map various column names dynamically using fuzzy search
                ...
                supervised_entity = find_val(["supervised entity", "banks name", "bank name"])
                sr_no = find_val(["sr.no.", "sr number", "serial number"])
                action_para_id = find_val(["action para id", "action para number"])
                observation = find_val(["observation", "action para description"])
                actual_due_date = find_val(["actual due date", "due date for compliance"])
                extended_due_date = find_val(["extended due date", "extended due date requested"])
                status = find_val(["status of observation as per the auditor", "status"])
                if not status:
                    status = find_val(["status"])

                # Gather all comments, SSM remarks, and responses for comprehensive vector indexing
                text_parts = [
                    ...
                    f"Auditor Status: {status}",
                ]

                # Append comments & SSM chains dynamically
                comments_found = []
                for key, val in rec.items():
                    if val and any(word in key.lower() for word in ["comment", "response", "reply", "remark"]):
                ...
                logger.error(
                    "Error parsing compliance record row: %s",
                    str(e),
                    exc_info=True
                )
                continue
        ...
        logger.info("Extracted %d compliance records from Excel", len(parsed_records))
